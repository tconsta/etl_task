# Copyright (c) 2019 Daniel Yule, The MIT License (MIT)
    ...
    def process_char(char, charcode):
        ...
        if state == __TOKENIZER_STATE.WHITESPACE:
            ...
            if char == "+" or char == "-" or char in "0123456789":
                ...
                raise ValueError(
                    "An e in a number must be followed "
                    "by a '+', '-' or digit.  Got '{0}'".format(char))
            ...
            if char in "0123456789":
                ...
                raise ValueError(
                    "A number exponent must consist only of digits.  Got '{}'".format(char))
            ...
            if is_delimiter(char):
                ...
                raise ValueError(
                    "Expected whitespace or an operator after strin.  Got '{}'".format(char))
        ...
        while True:
            if isinstance(stack[-1], list):
                    ...
                    if last_token == "[":
                        if token_type == TOKEN_TYPE.OPERATOR:
                            if token == "{":
                                ...
                                raise ValueError("Array must either be empty or contain a value.  "
                                                 "Got '{}'".
                                                 format(token))
                        else:
                            stack.append(token)
                        ...
                        if isinstance(stack[-1], list):
                            ...
                            raise ValueError("Array items must be followed "
                                             "by a comma or closing bracket.  "
                                             "Got '{}'".format(value))
                    elif last_token == "}":
                    ...
                    if last_token == "{":
                        if token_type == TOKEN_TYPE.OPERATOR:
                            if token == "{":
                                ...
                                raise ValueError("Object must either be empty "
                                                 "or contain key value pairs."
                                                 "  Got '{}'".format(token))
                        ...
                        if isinstance(stack[-1], list):
                            ...
                            if isinstance(stack[-1], list):
                                ...
                                raise ValueError("Object key value pairs must be followed "
                                                 "by a comma or closing bracket.  "
                                                 "Got '{}'".format(value))
                    elif last_token == "]":
                ...
                if stack[-1].set:
                    if token_type == TOKEN_TYPE.OPERATOR:
                        if token != "}" and token != ",":
                            raise ValueError("Object key value pairs should be "
                                             "followed by ',' or '}'. "
                                             "Got '" + token + "'")
                        value = stack.pop()
                        if len(stack) == 0:
                        ...
                        if isinstance(stack[-1], list):
                            ...
                            raise ValueError(
                                "Object key value pairs must be followed "
                                "by a comma or closing bracket. "
                                "Got '{}'".format(value))
                        ...
                        raise ValueError(
                            "Object key value pairs should be followed "
                            "by ',' or '}'.  Got '" + token + "'")
                else:
                    if token_type == TOKEN_TYPE.OPERATOR and \
                       token == ":" and last_type == TOKEN_TYPE.STRING:
                        pass
                        ...
                        if token_type == TOKEN_TYPE.OPERATOR:
                            if token == "{":
                                ...
                                raise ValueError(
                                    "Object property value expected.  Got '{}'".format(token))
                        else:
                            stack[-1].value = token
                            stack[-1].set = True
                    else:
    ...
    def process_token(token_type, token):
        if token_type == TOKEN_TYPE.OPERATOR:
            if token == ']':
                ...
                raise ValueError(
                    "Array entries must be followed by ',' or ']'.  Got '{}'".format(token))
        else:
            return token, None, None
